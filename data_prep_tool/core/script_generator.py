        ...
        if needs_binary_label_normalization:
            self._append_guarded_step(
                ...
                [
                    ...
                    "        numeric_values = set(pd.to_numeric(series.dropna().unique(), errors='coerce'))",
                    "        if numeric_values and numeric_values <= {0, 1}:",
                    f"            df[col_name] = series.replace({{1: {self._s(self.binary_true)}, 0: {self._s(self.binary_false)}}})",
                    "        continue",
